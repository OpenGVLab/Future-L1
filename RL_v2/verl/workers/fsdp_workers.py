...
# ----------------------------------------------------------------------
# CRITICAL: apply the FutureL1 vLLM runner patch **inside the Ray worker
# process** before any vllm.v1.worker module is imported. This file is
# imported by every WorkerDict actor that Ray spawns; without this hook
# the workers would use the stock vLLM GPUModelRunner (no latent state
# machine), which the upstream RL/future_l1_rl works around the same way.
# Mirrors HyLar/RL/verl/workers/fsdp_workers.py:17.
# ----------------------------------------------------------------------
import os as _os
import sys as _sys

if _os.environ.get("FUTURE_L1_RL_PATCH", "1") != "0":
    _rl_v2_root = _os.path.abspath(_os.path.join(_os.path.dirname(__file__), _os.pardir, _os.pardir))
    ...
    try:
        ...
        _msg = f"[verl.workers.fsdp_workers] FutureL1 patch skipped: {_e}"
        ...
        if _os.environ.get("FUTURE_L1_RL_PATCH_REQUIRED", "0") == "1":
...
class FSDPWorker(Worker):
    def __init__(
        ...
        if not dist.is_initialized():
            # Default PG timeout is 10 min, which is too short for video RL:
            # long-tail video decoding / s3 reads can stall a single rank for
            # several minutes, causing the NCCL watchdog to abort the job.
            # Allow override via NCCL_PG_TIMEOUT_MIN env (default 60 min).
            from datetime import timedelta
            _pg_timeout_min = int(_os.environ.get("NCCL_PG_TIMEOUT_MIN", "60"))
    ...
    def _process_multi_modal_inputs(self, data: DataProto):
        ...
        if "multi_modal_inputs" not in self._cache:
            ...
            image_min_pixels = data.meta_info.get("image_min_pixels", min_pixels)
            image_max_pixels = data.meta_info.get("image_max_pixels", max_pixels)
            video_min_pixels = data.meta_info.get("video_min_pixels", min_pixels)
            video_max_pixels = data.meta_info.get("video_max_pixels", max_pixels)
            video_fps = data.meta_info["video_fps"]
            max_video_frames = data.meta_info.get("max_video_frames", None)
            ...
            for index, multi_modal_data in zip(
                ...
                if index not in multi_modal_inputs_cache:
                    ...
                    if len(images) != 0 and len(videos) != 0:
                        # joint video + image sample (e.g. SEED-Bench-R1)
                        img_inputs = dict(self.processor.image_processor(images=images, return_tensors="pt"))
                        video_processor = getattr(self.processor, "video_processor", None)
                        if video_processor is not None:
                            vid_inputs = dict(video_processor(videos=videos, return_tensors="pt"))
                        else:
                            vid_inputs = dict(
                                self.processor.image_processor(images=None, videos=videos, return_tensors="pt")
                            )
                        multi_modal_inputs = {**img_inputs, **vid_inputs}
                    elif len(images) != 0:
                        ...
                        video_processor = getattr(self.processor, "video_processor", None)
                        if video_processor is not None:
                            multi_modal_inputs = dict(video_processor(videos=videos, return_tensors="pt"))
                        else:
    ...
    def offload_for_external_eval(self):
        """Free GPU memory so a separate lmms_eval process can load the model."""
        if self._has_rollout:
            self.release_rollout_engine()
        if self._has_actor and hasattr(self, "fsdp_module"):
            offload_fsdp_model(self.fsdp_module)
        if self._has_ref and hasattr(self, "ref_fsdp_module"):
            offload_fsdp_model(self.ref_fsdp_module)

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def reload_after_external_eval(self):
        if self._has_actor and hasattr(self, "fsdp_module"):
            load_fsdp_model(self.fsdp_module)
        if self._has_ref and hasattr(self, "ref_fsdp_module"):
