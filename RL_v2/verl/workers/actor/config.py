...
class ActorConfig:
    ...
    """disable per-phase tqdm bars inside actor workers"""
    ulysses_size: int = 1
    ...
    # ===== FutureL1 / DePO fields =====
    future_l1_rl_kappa: float = 0.01
    """vMF concentration kappa; same scale as HyLar's hylar_rl_kappa."""
    enable_decoupled_hybrid_ppo: bool = False
    """D-1: split token/latent PPO losses (auto-keyed from algorithm.*)."""
    ...
    """D-2: closed-form vMF KL constraint at latent positions."""
    latent_kl_coef: float = 1e-2
    enable_format_gated_latent_loss: bool = False
    """Only apply latent PPO / vMF KL losses to format-valid samples."""
    ...
    # Sampling strategy propagated from rollout config; tells the actor
    # whether to expect rolled-out latents and apply DePO bookkeeping.
    sampling_strategy: str = field(default="default", init=False)
...
class RefConfig:
    ...
    fsdp: FSDPConfig = field(default_factory=FSDPConfig)
    offload: OffloadConfig = field(default_factory=OffloadConfig)
    # FutureL1 kappa propagated from actor for ref-policy log-prob too.
    future_l1_rl_kappa: float = 0.01
    # below are auto keys
