...
class BatchFunctionRewardManagerMixin:
    ...
    def compute_reward_batch(self, data: DataProto) -> Tuple[torch.Tensor, dict[str, list[float]]]:
        ...
        response_ids = data.batch["responses"]
        response_length = torch.sum(data.batch["response_mask"], dim=-1)
        # Optional fields: the question text (used by LLM-as-judge rewards
        # such as FutureL1's, where the judge sees task context) and a
        # per-sample reference response length (used by length-penalty
        # rewards). Both are looked up by key so the manager stays a no-op
        # when the dataset/rollout do not produce them.
        problems = data.non_tensor_batch.get("problem")
        ref_resp_lengths = data.non_tensor_batch.get("ref_resp_lengths")
        latents = data.non_tensor_batch.get("latents")
        ...
        for i in range(len(data)):
            ...
            if problems is not None:
                try:
                    ...
                    pass
            if ref_resp_lengths is not None:
                try:
                    item["ref_resp_length"] = int(ref_resp_lengths[i])
                except (TypeError, ValueError):
                    pass
            if latents is not None:
                try:
                    item["latents"] = latents[i]
                except Exception:  # noqa: BLE001
                    pass
            if uids is not None:
                try:
