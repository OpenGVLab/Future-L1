...
class AdvantageEstimator(str, Enum):
    ...
    REMAX = "remax"
    RLOO = "rloo"
    DAPO = "dapo"  # alias of GRPO outcome advantage; paired with online filtering.
...
def compute_dapo_outcome_advantage(
    ...
    """Compute advantage for DAPO.

    DAPO uses the same outcome advantage as GRPO; the algorithmic difference
    (online filtering of groups, asymmetric clip ratios) is enforced at the
    trainer / actor level, not in the advantage formula.
    """
...
# ----------------------------------------------------------------------
# FutureL1 / DePO: closed-form vMF KL constraint at latent positions.
# Mirrors HyLar's compute_latent_vmf_kl: with shared concentration kappa,
#   KL[vMF(mu_actor, k) || vMF(z_rollout, k)] ≈ k * (1 - <mu_actor, z_rollout>)
# where <.,.> is the unnormalised inner product (the FutureL1 projection head
# already produces dimension-matched vectors, so we keep this consistent with
# the rollout path).
# ----------------------------------------------------------------------
def compute_latent_vmf_kl(
    mu_actor: torch.Tensor,
    mu_ref: torch.Tensor,
    kappa: float = 0.01,
) -> torch.Tensor:
    """vMF KL constraint at latent positions.

    Args:
        mu_actor: ``(L, D)`` actor hidden states at latent positions
            (gradient-enabled when called from the policy update).
        mu_ref:   ``(L, D)`` reference vectors at latent positions, typically
            the rollout-time projected hidden state (``z_rollout``).
        kappa:    vMF concentration. Higher = tighter constraint.

    Returns:
        Scalar tensor: mean ``kappa * (1 - cos)`` over latent positions.
    """
    mu_a = mu_actor.float()
    mu_r = mu_ref.float()
    cos_sim = (mu_a * mu_r).sum(dim=-1)  # (L,)
    return (kappa * (1.0 - cos_sim)).mean()
